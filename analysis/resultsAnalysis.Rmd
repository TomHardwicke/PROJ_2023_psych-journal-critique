...
dj_prominent %>%
  ...
  print()
```
Get how many prominent journals offered more than one type of PPC
```{r}
...
randomPPC_CI
```
Get how many randomly selected journals offered more than one type of PPC
```{r}
# Return all rows with ppc_name values for ppc_type B and C
dj_random %>%
  filter((ppc_type == "B" | ppc_type == "C") & (!is.na(ppc_name)))
...
quantLength %>%  
 ...
 coord_flip() +
 theme_bw() +
 labs(Title = "Length Limits", x = "Post-publication critique type", y = "Length limits (words)")
...
quantTime %>%  
 ...
 coord_flip() +
 theme_bw() +
 labs(Title = "Time Limits", x = "Post-publication critique type", y = "Time limits (weeks)")
```
