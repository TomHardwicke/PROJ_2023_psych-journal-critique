...
There is some information about the sample that isn't included in the extracted data files (sampleID, JIFs, Web of Science subject category, COPE status), so we need to load some of that information from the original sample files and attached it to the extracted data
...
For the random journals we can find the JIFs in the prominent journal sample. Some journals do not have a JIF because they haven't been indexed long enough. We manually checked all of these journals on Journal Citation Reports to see if they had been assigned JIFs. One JIF needs to be added manually for the journal "Journal of Psychology and Theology". The 2021 JIF for this journal is .80.
...
# get JIF and WOS subject category for prominent journals
journal_list_prominent_all <- journal_list_prominent_all %>%
  select(journal = `Journal name`, `2021 JIF`, WOS_first_psych_category) # identify the columns we need from the sample data

d_journals_prominent <- left_join(d_journals_prominent, journal_list_prominent_all, by = 'journal') # attach info to extracted data

# get WOS subject category for random journals
journal_list_random_all <- journal_list_random_all %>%
  select(journal = Journal, WOS_first_psych_category) # identify the columns we need from the sample data

d_journals_random <- left_join(d_journals_random, journal_list_random_all, by = 'journal') # attach info to extracted data
...
journal_list_prominent_all <- journal_list_prominent_all %>%
  select(-WOS_first_psych_category) # we don't this column
  
d_journals_random <- left_join(d_journals_random, journal_list_prominent_all, by = 'journal') # attach info to extracted data

d_journals_random <- d_journals_random %>%
  mutate(`2021 JIF` = ifelse(
    journal == "JOURNAL OF PSYCHOLOGY AND THEOLOGY", # for one journal,
    .80, # manually add its JIF
    `2021 JIF`)) # otherwise keep whatever is already in the JIF column

# reunite d_journals_random and d_journals_prominent
d_journals <- bind_rows(
  d_journals_random %>% mutate(sampleID = "random"),
  d_journals_prominent %>% mutate(sampleID = "prominent")
)
rm(d_journals_random)
rm(d_journals_prominent)
```

#Add COPE status FROM PRACTICE CODING, does not alter main d_journals tibble
```{r}
...
d_primaryONLY <- left_join(d_primaryONLY, dj_cope, by = 'journal')
...
```{r}
d_journals_final_eligible <- d_journals_final_eligible %>% 
  mutate(review_harmonized = case_when(
