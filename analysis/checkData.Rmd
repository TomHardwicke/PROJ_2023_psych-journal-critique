...
journal_list_prominent_all <- read_csv(here('data','prepareSample','journals', '03 - final', 'journals-prominent.csv'))

# list of all random journals (some not in sample)
journal_list_random_all <- read_csv(here('data','prepareSample','journals', '03 - final', 'journals-random.csv'))
```
...
Adjust two journal names entered incorrectly so they match names in sample.

```{r}
d_journals <- d_journals %>% mutate(journal = fct_recode(journal, 
...
Check journals where we have more than 2 entries

```{r}
d_journals_prominent %>% 
  count(journal) %>%
  filter(n > 2)
```
...
```{r}
d_journals_prominent <- d_journals_prominent %>%
