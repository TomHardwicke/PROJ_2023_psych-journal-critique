...
d_journals_final_eligible <- d_journals_final_eligible %>% 
  mutate(length_harmonized = str_remove_all(length, pattern = '"'), #  remove any quotation marks
         length_harmonized = str_remove(wordLimits, pattern = fixed("YES (copy and paste the limits in the 'other' section below), "))), # remove data entry question text
```
...
Firstly, specify PPC names that we want to convert to different haromized names.

```{r}
commentaries <- c("Brief comment", "Brief Comment")
letters <- c(
...
```{r}
d_journals_final_eligible %>%
  mutate(name_harmonized = )
```

In Emotion, one line seems to be about ten words. So 260 lines is 2600 words.
