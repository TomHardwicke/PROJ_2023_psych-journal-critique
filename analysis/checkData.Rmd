...
Now check that when `exclusion_primary` is "RETAIN" `exclusion_reason` is NA.

```{r}
d_articles <- d_articles %>%
  mutate(exc_col_check = ifelse(exclusion_primary == "RETAIN" & is.na(exclusion_reason_primary), T, F)) # if we are retaining there should be an NA in the reason column
  
assert_that(all(d_articles$exc_col_check == F), msg = "Found unexpected value in exclusion reason column when it should be NA")
```
